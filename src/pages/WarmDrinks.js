...
import "./WarmDrinks.css";

function WarmDrinks() {
  const { t } = useTranslation();
  const navigate = useNavigate();

  // Get translated data from warmDrinks namespace
  const title = t("title", { ns: "warmDrinks" });
  const backToMenu = t("backToMenu", { ns: "warmDrinks" });
  const categories = t("categories", { ns: "warmDrinks", returnObjects: true });
